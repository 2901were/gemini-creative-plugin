...
# Gemini Workflow Selection Guide
...
**Why the `images` param is critical:** When `send_creative_message` receives `images[]`, it passes the actual pixel data to Gemini as visual input — functionally identical to `edit_image`. Without it, Gemini reconstructs from text alone, causing proportion drift, color changes, and detail inconsistencies.

**Validated results (October 2025):**
- ✅ Perfect helmet/armor consistency across poses
- ✅ Correct proportions (no dwarf/chibi effect)
...
**Do NOT** ask Gemini to compose the final mockup (phone frames + labels + layout). It introduces text rendering errors and layout distortions.
