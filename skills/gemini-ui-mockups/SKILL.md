...
# Gemini UI Mockup Series Generator

Generate a consistent set of UI mockup screens with a unified style using the Hybrid Workflow.

## Key Rules

- Generate individual screens separately (do NOT ask Gemini to compose phone frames + labels — it introduces text errors like "SWIM" → "SVIM")
...
2. **Generate 3 style variations of Screen 1** using `generate_image` (no session needed for style selection):
   ...
   - Use `responseMode: "text_and_image"` so the model describes what it generated
...
5. **Generate each subsequent screen** using `send_creative_message`:
   ...
   - Screen 4: `images: [screen1_path, screen2_path, screen3_path]` — up to 14 refs with Nano Banana 2
...
When passing image references via `images[]`, Gemini treats the **entire** image as canonical visual content. Anything visible in the ref — including accidents like palette legends, debug overlays, watermarks, or unintended UI elements — propagates into the output.

**Documented failure (2026-05-12, production-pass D6):** a three-phone composed-mockup generation passed prior validation-experiment screens as refs. Those refs had palette legends rendered into them. All three generated phones came back with the palette legend bled through into each phone screen.
...
When the prompt requests a specific grid layout (e.g., `"4×6 grid of inventory slots"`), Gemini fits the grid to the available canvas regardless of explicit dimension counts. **Pick the aspect ratio that natively fits the requested grid**, not the platform's display aspect.
