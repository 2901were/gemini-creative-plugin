...
**Consistency rule:** Every distinctive detail must be named explicitly in the first image. Subsequent poses pass the previous image visually — details propagate automatically via the `images` parameter. See the `gemini-game-assets` skill for the full resolution guide, palette table, and isometric rules.
